...
The fluctuations observed by ISR are associated with two longitudinal wave modes in the plasma: Êthe ion-acoustic mode and the Langmuir mode. Ê Most ISR investigations involve the use multiple pulses to create an estimate of the ion acoustic spectrum (i.e., the Fourier Transform of the ACF) which is then fit to a theoretical model in order to estimate ionospheric state parameters (typically plasma density, electron temperature, ion temperature, and bulk line-of-site plasma motion). The theory of how the plasma parameters impact the statistics of these fluctuations have been discussed since the first use of this sensor modality \citep{gordon58,dougherty:farley1960, farleydougherty:ISR2, doughteryfarley:ISR3, hagfors1961}, and even as recent as 2011 there have been new formulations of this theory \citep{kudeki:milla:1,kudeki:milla:2}. 
...
he simulator takes as input a discretized set of ionosphere state parameters in CartesianÊcoordinates which may vary in time. ÊThis corresponds to the true field we will seek to reconstruct. Ê The first step in the simulator is to create theoretical ISR spectra at each point from the prescribed parameters. ÊFor details on calculating these spectra from the intrinsic plasma parameters see \citet{kudeki:milla:1} and \citet{kudeki:milla:2}. 
...
The final result of the this summation rule will still lead to a biased ACF. For the long pulse case this summation rule leads to theÊ following expected value for the estimator \citep{nygren1996},
...
After the final estimation of the spectrum is complete the nonlinear least squares fitting takes place to determine the parameters.  The class of nonlinear least squares problems relevant to ISR parameter estimation can be represented as the Êminimization of a cost function of the form \citep{kayvol1},
