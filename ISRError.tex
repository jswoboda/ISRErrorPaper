...
The beam shapes needed for the antennas are creating using standard theory for phased arrays and dish antennas. For the circular dish antennas the term $F(\theta_s,\phi_s,\theta,\phi)$ will be shown in a rotated frame system where the pointing positions $\theta_s$ and $\phi_s$ will be both zero 0 degrees in azimuth and elevation respectively and $\theta_r$ and $\phi_r$ will be $\theta$ and $\phi$ in this frame. This then leads to the following formulation from \citet{Blahut:2004wd}:

\begin{equation}
\label{eqn:circantenna}
F(\theta_r,\phi_r)=\frac{4r^2}{\lambda^2}\text{jinc}\left(\frac{2r}{\lambda}\sin{\phi_r}\right),
\end{equation}
where $r$ is the radius of the antenna in meters (m), $\lambda$ is the wavelength of the radar in meters (m). The jinc function is defined as the following,
\begin{equation}
\label{eqn:jinc}
...
\noindent where $\alpha(m) = \sqrt{P_r(m)}/\widehat{\sigma}_y$ and $\sigma_y$ is the standard deviation of $y_m(i)$ and the bracket denotes an estimator. Lastly, to model total noise from the radar system and environment, an additive CWGN process is included, creating the final simulated complex receiver voltage sequence
...
Finally, noise effects are included by subtracting an estimate of the noise correlation from $\widehat{R}_s(m,l)$.  We represent the noise correlation function as $\widehat{R}_w(m,l)$, the ACF estimate of the background noise process of the radar $w(n_w)$ using the steps in Equations \ref{eq:lagpro} and \ref{eq:sumrule}. In a real radar system the noise process is typically sampled either during a calibration period for the radar when nothing is being emitted, or at ranges sufficiently distant that scattered ionospheric signal is assumed to be negligible. If there are localized but large returns, for example coherent reflections from satellites, this impact can be reduced by applying estimators that use order statistics \citep{ordstatcfar}. For our simulated estimate, we simply use a noise process with the same correlation structure and power as the noise that was added. The final estimate of the autocorrelation function after the noise subtraction and summation rule is represented by $\widehat{R}_f(m,l)$. 
...
%MZ - Doesn't the fitter have 'rails' to prevent negative temperature results?
%JPS - I have a constraint in the fitting function that heavily penalizes negative temperatures. I talked with Mike Nicolls about this, he suggested making the constraints continuous functions as the fit is a gradient search.
  
%enhancements in ion temperature is captured in the reconstruction, although  do look wider than the input which is likely due to the spatial radar ambiguity function. 
...
%% PJE: I don't buy this last sentence.  ISR is a statistical process.  One realization does not mean anything - you said it yourself in an earlier section.  How can you 'fully explore' a process that you cannot statistically resolve, no matter how clever the design?  I think this sentence needs rewording/refocusing - or you could be simple and just eliminate it completely.
%% JPS: Added a statement about novel integration schemes.
