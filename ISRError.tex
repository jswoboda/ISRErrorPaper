...
The sources of error from electronically steerable array (ESA) incoherent scatter radar (ISR) are investigated both theoretically and with use of an open source ISR simulator, developed by the authors. The main sources of error that will be explored are from the statistical uncertainty, which arises due to nature of the measurement mechanism, and the inherent space-time ambiguity from the sensor. The simulator under discussion can take a field of plasma parameters and create simulated ISR data at the in-phase and in-quadrature (I and Q) level and then process it to show a possible reconstruction of the parameters field. To show the utility of the simulator a number of examples will be shown, with one case using data from a self-consistent multi-ionic fluid transport model. This will demonstrate the impact the forward model of the ISR and statistical uncertainty on the parameter measurements, and the core trade offs that must be made when planning experiments. Thus, highlighting the need for this type of simulator to iterate through different experiment set ups.
